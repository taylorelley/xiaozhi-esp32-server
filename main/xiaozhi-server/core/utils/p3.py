...
def decode_opus_from_file(input_file):
    """
    Decode Opus data from a p3 file, returning a list of Opus packets and the total duration.
    """
    opus_datas = []
    total_frames = 0
    ...
    with open(input_file, 'rb') as f:
        while True:
            # Read the header (4 bytes): [1 byte type, 1 byte reserved, 2 bytes length]
            header = f.read(4)
            if not header:
                break
            ...
            # Read the Opus data based on the length in the header
...
def decode_opus_from_bytes(input_bytes):
    """
    Decode Opus data from p3 binary data, returning a list of Opus packets and the total duration.
    """
    import io
    opus_datas = []
    total_frames = 0
