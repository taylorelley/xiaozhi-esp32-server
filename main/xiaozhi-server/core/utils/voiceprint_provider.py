...
class VoiceprintProvider:
    ...
    def __init__(self, config: dict):
        ...
        # Voiceprint similarity threshold; defaults to 0.4
        ...
        if not self.original_url:
            ...
            # Extract the key from the query string
            ...
            if not self.api_key:
                ...
                # Build the identify endpoint URL
                ...
                # Check whether any valid speaker is configured
                if not self.speaker_ids:
                    ...
                    if self._check_server_health():
                        ...
                        self.enabled = False
                        logger.bind(tag=TAG).warning(f"Voiceprint recognition server is unavailable; voiceprint recognition is disabled: {self.api_url}")

    def _parse_speakers(self) -> Dict[str, Dict[str, str]]:
        """Parse the speaker configuration."""
        speaker_map = {}
        for speaker_str in self.speakers:
            try:
                parts = speaker_str.split(",", 2)
    ...
    def _check_server_health(self) -> bool:
        """Check the health of the voiceprint recognition server."""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("status") == "healthy":
                    ...
                    logger.bind(tag=TAG).warning(f"Voiceprint recognition server is in an abnormal state: {result}")
                    is_healthy = False
            else:
            ...
            logger.bind(tag=TAG).warning("Voiceprint recognition server refused the connection")
            is_healthy = False
        except Exception as e:
    ...
    async def identify_speaker(self, audio_data: bytes, session_id: str) -> Optional[str]:
        ...
        if not self.enabled or not self.api_url or not self.api_key:
            logger.bind(tag=TAG).debug("Voiceprint recognition is disabled or unconfigured; skipping identification")
            return None
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(self.api_url, headers=headers, data=data) as response:
                    ...
                    if response.status == 200:
                        ...
                        logger.bind(tag=TAG).info(f"Voiceprint recognition elapsed: {total_elapsed_time:.3f}s")

                        # Similarity threshold check
                        if score < self.similarity_threshold:
                            logger.bind(tag=TAG).warning(f"Voiceprint similarity {score:.3f} is below the threshold {self.similarity_threshold}")
