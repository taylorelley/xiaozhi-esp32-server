...
class OTAHandler(BaseHandler):
    ...
    def generate_password_signature(self, content: str, secret_key: str) -> str:
        ...
        Args:
            content: The content to sign (clientId + '|' + username).
    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            if device_id:
                ...
                raise Exception("OTA request has an empty device ID")

            client_id = request.headers.get("client-id", "")
            if client_id:
                self.logger.bind(tag=TAG).info(f"OTA request ClientID: {client_id}")
            else:
                raise Exception("OTA request has an empty ClientID")
            ...
            if mqtt_gateway_endpoint:  # A non-empty string is configured
                # Try to derive the device model from the request data (already parsed above)
                try:
                ...
                if signature_key:
                    ...
                    if not password:
                        password = ""  # Leave empty on signing failure; the device decides whether to allow passwordless auth
                else:
                    self.logger.bind(tag=TAG).warning("Missing MQTT signing key; password left empty")

                # Build the MQTT config (using the mqtt_gateway string directly)
                return_json["mqtt"] = {
                ...
                self.logger.bind(tag=TAG).info(f"Sent MQTT gateway configuration to device {device_id}")

            else:  # No mqtt_gateway configured; fall back to WebSocket
                # If authentication is enabled, run the auth check
                token = ""
                if self.auth_enable:
                ...
                self.logger.bind(tag=TAG).info(
                    f"MQTT gateway not configured; sent WebSocket configuration to device {device_id}"
                )
            ...
            try:
                ...
                self.logger.bind(tag=TAG).info(
                    f"Looking up firmware for model {device_model}; found {len(candidates)} candidate(s)"
                )
                ...
                if chosen_url:
                    ...
                    self.logger.bind(tag=TAG).info(
                        f"Sent firmware {chosen_version} to device {device_id} [if the URL prefix is wrong, check server.vision_explain in the config file] -> {chosen_url} "
                    )
                else:
                    self.logger.bind(tag=TAG).info(
                ...
                self.logger.bind(tag=TAG).error(f"Error while checking firmware version: {e}")
            ...
            self.logger.bind(tag=TAG).error(f"OTA POST handler exception: {e}")
    ...
    async def handle_get(self, request):
        ...
        try:
            ...
            message = f"OTA endpoint is running normally. The WebSocket URL sent to the device is: {websocket_url}"
    ...
    async def handle_download(self, request):
        ...
        - Only .bin files under data/bin may be downloaded.
