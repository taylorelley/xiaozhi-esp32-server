    ...
    def get_all_tools(self) -> Dict[str, ToolDefinition]:
        ...
        for tool_type, executor in self.executors.items():
            try:
                ...
                self.logger.error(f"Error while retrieving {tool_type.value} tools: {e}")

        self._cached_tools = all_tools
        return all_tools

    def get_function_descriptions(self) -> List[Dict[str, Any]]:
        """Get the function descriptions for all tools (OpenAI format)"""
    ...
    def get_tool_statistics(self) -> Dict[str, int]:
        ...
        for tool_type, executor in self.executors.items():
            try:
                ...
                self.logger.error(f"Error while retrieving {tool_type.value} tool statistics: {e}")
