"""IoT device support module, providing IoT device descriptors and state handling"""
...
async def handleIotDescriptors(conn: "ConnectionHandler", descriptors):
    ...
    for descriptor in descriptors:
        # If the descriptor has neither properties nor methods, skip it directly
        if "properties" not in descriptor and "methods" not in descriptor:
            continue

        # Handle the case of missing properties
                ...
                for method_name, method_info in descriptor["methods"].items():
                    if "parameters" in method_info:
                        for param_name, param_info in method_info["parameters"].items():
                            # Convert the parameter information into property information
                            descriptor["properties"][param_name] = {
                                "description": param_info["description"],
                                "type": param_info["type"],
                            }

        # Create an IoT device descriptor
    ...
    # If new functions have been registered, update the function description list
                        ...
                        if property_item["name"] == k:
                            if type(v) != type(property_item["value"]):
                                logger.bind(tag=TAG).error(
                                    f"Value type of property {property_item['name']} does not match"
                                )
                                break
                            else:
