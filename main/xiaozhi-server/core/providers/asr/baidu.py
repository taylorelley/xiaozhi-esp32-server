    ...
    async def speech_to_text(
        ...
        try:
            ...
            if not self.app_id or not self.api_key or not self.secret_key:
                logger.bind(tag=TAG).error("Baidu speech recognition not configured; cannot recognize")
                return None, None
            ...
            if result and result["err_no"] == 0:
                ...
                raise Exception(
                    f"Baidu speech recognition failed, error code: {result['err_no']}, message: {result['err_msg']}"
                )
