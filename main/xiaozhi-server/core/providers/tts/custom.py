...
class TTSProvider(TTSProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        if isinstance(self.params, str):
            try:
                ...
                raise ValueError("Invalid Custom TTS configuration parameters: cannot parse the string as an object")
        elif not isinstance(self.params, dict):
            raise TypeError("Invalid Custom TTS configuration parameters; please refer to the configuration documentation")
    ...
    async def text_to_speak(self, text, output_file):
        ...
        if resp.status_code == 200:
            ...
            raise Exception(error_msg)  # Raise the exception for the caller to handle
